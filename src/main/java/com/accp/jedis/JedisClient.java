...
public interface JedisClient {
	...
	/**
	 * 同一个事务setnx保存，然后设置过期时间，异常返回null，已存在key则返回0并不更新value
	 * 
	 * @param key
	 * @param value
	 * @param seconds
	 * @return
	 */
	Long setNxAndEx(String key, String value, int seconds);

	/**
	 * 以秒为单位，返回给定 key 的剩余生存时间
	 * 
	 * @param key
	 * @return 返回值： 当 key 不存在时，返回 -2 。 当 key 存在但没有设置剩余生存时间时，返回 -1 。 否则，以秒为单位，返回 key
	 *         的剩余生存时间。
	 */
	Long ttl(String key);
